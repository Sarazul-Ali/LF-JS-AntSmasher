 ...
 * GAME PART
 *
 * ballCount - no. of ant needed,
 * minBallSize - minimum size for ant,
 * maxBallSize - maximum size for ant,
 * maxSpeed - maximum speed of ant movement
 * minspeed - minimum speed of ant movement
 *
 */
...
function AntGame(
    ...
    this.app = document.getElementById('antApp');
 ...
 */
AntGame.prototype.createAntBoxContainer = function() {
...
AntGame.prototype.createAnt = function() {
 ...
 */
AntGame.prototype.moveAntsInterval = function() {
    setInterval(this.moveAnts.bind(this), 1000 / 60);
};
...
AntGame.prototype.moveAnts = function() {
 ...
 */
function Ant(parentElement, top, left, radius, speed, angle) {
    ...
    this.antDiv = document.createElement('img');
}

/**
 * Ball init
 */
Ant.prototype.antInit = function() {
    this.antDiv.style.position = 'absolute'
    this.antDiv.style.width = this.radius * 2 + 'px';
    this.antDiv.style.height = this.radius * 2 + 'px';
    this.antDiv.setAttribute('src', 'images/ant.gif');

    this.parentElement.appendChild(this.antDiv);
...
Ant.prototype.drawAnt = function() {
    this.antDiv.style.left = this.left + 'px';
    this.antDiv.style.top = this.top + 'px';
};

/**
 * move ball
 */
Ant.prototype.moveAnt = function() {
 ...
 * start game
 */
AntGame.prototype.antGameInit = function() {
    this.createAntBoxContainer();
    this.createAnt();
    this.moveAntsInterval();
};
